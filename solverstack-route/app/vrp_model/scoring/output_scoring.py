...
def get_total_distance(
    ...
    :matrix:          list of lists of travel distances between nodes
                      (includes origin)
    :include_origin:  include travel distance from and to origins in
                      calculation
...
def get_stop_distance_factor(
    ...
    """
    Calculate average stop travel distance not including origin depart or
    return.
    len(vehicles) == len(stops) == len(matrix[1:]li[1:]). matrix contains
...
def get_route_distance_factor(
    ...
    """
    Calculate average vehicle route travel distance including origin depart
    and return.
    len(vehicles) == len(stops) == len(matrix[1:]li[1:]). matrix contains
    origin node position.
